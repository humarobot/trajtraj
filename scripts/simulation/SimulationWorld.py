...
class SimulationWorld(ISimulationWorldBase):
    ...
    def get_joint_states_at(self, robot_id, trajectory, group):
        self.reset_joint_states_to(robot_id, trajectory, group)
        joint_states = sim.getJointStates(robot_id, list(range(sim.getNumJoints(robot_id))))
    ...
    def execute_trajectory(self, robot, trajectory, step_time=0.5):
        for each_time_step_trajectory in trajectory.final:
            self.reset_joint_states_to(robot.id, each_time_step_trajectory, list(trajectory.trajectory_by_name.keys()))
    ...
    def setup_joint_id_to_joint_name(self, robot_id):
        ...
        self.joint_id_to_name = OrderedDict(list(zip(list(self.joint_name_to_id.values()), list(self.joint_name_to_id.keys()))))
        ...
        while (True):
            ...
            for i in range(len(param_ids)):
                ...
                if param == print_pose_id:
                    if param_value > 0:
                        data = OrderedDict(list(zip(group, target_pos)))
                        data = {"loc" + str(i): data}
                        if not wrote:
